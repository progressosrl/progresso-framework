...
namespace Progresso\Framework\Routing;
...
class Router
{
    ...
    public static function hooks()
    {
        ...
        add_filter('posts_pre_query', self::class . '::postsPreQuery', 10, 2);
    ...
    public static function route()
    {
        ...
        if (isset(static::$match['handler']) && \is_callable(static::$match['handler'])) {
            \call_user_func(static::$match['handler'], static::$match['vars']);
        }
        // if it is a castom route, it has not to continue with normal wordpress flow
        if (static::$match['type'] == 'custom') {
            exit;
        }
    }
    /**
