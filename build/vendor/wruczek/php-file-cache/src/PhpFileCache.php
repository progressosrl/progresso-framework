...
namespace Progresso\Framework\Vendor\Wruczek\PhpFileCache;
...
class PhpFileCache
{
    ...
    public function store($key, $data, $expiration = 60, $permanent = \false)
    {
        ...
        $storeData = ["time" => \time(), "expire" => $expiration, "data" => $data, "permanent" => $permanent];
     ...
     * @throws \Exception if the file cannot be saved
     */
    public function isExpired($key, $eraseExpired = \true)
    {
        if ($eraseExpired) {
            $this->eraseExpired();
        }
     ...
     * @throws \Exception if the file cannot be saved
     */
    public function isCached($key, $eraseExpired = \true)
    {
        if ($eraseExpired) {
            $this->eraseExpired();
        }
